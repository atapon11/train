          ...
          if (response.recordset) {
            ...
            if (search) {
              query = query.filter((a) =>
                a.treatment_type_name.includes(search)
              ); //คำสั่ง เสิร์ช เมื่อเสิร์ชตัวคิวรี่ มีค่าถูกส่ง ให้ทำตามเงื่อนไข  treatment_type_name  includes จะทำในวงเล็บ search ถ้าตรงกันมันจะ true ถ้าไม่ตรงจะ false
            }
            if (status) {
              query = query.filter(
                (a) => a.is_used.toString() === status.toString()
              );
            }
            res
              .status(200)
...
router.get("/getDetailTreatment/:id", async function (req, res) {
  try {
    ...
    await mssql.sql.query(
      `SELECT id, treatment_type_name AS name FROM treatment_type WHERE id = '${req.params.id}'`,
      function (err, response) {
        if (response) {
          if (response.recordset) {
            var query = response.recordset;
...
router.post('/createTreatment', async function (req, res){
    try {
        ...
        await mssql.sql.query(
            `INSERT INTO treatment_type (treatment_type_name, created_date, is_used) VALUES ('${req.body.name}',GETDATE(),1)`,
            function (err, response){
                if(response){
